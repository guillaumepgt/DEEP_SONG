 ...
 * @date    11 mars 2026
 * @brief   Implémentation des fonctions graphiques et des fenêtres de l'IHM.
 *******************************************************************************
 */
...
/**
 * @brief  Modèle générique interne pour l'affichage de fenêtres textuelles contextuelles.
 * @param  title Chaîne de caractères affichée dans le bandeau supérieur.
 * @param  line1 Ligne de texte principale au centre du cadre.
 * @param  line2 Ligne de texte secondaire (optionnelle, passer 0 si inutilisée).
 * @return Aucun.
 */
static void DISPLAY_DrawMessageScreen(char *title, char *line1, char *line2)
{
    ILI9341_Fill(C_BACKGROUND);
...
{
    DISPLAY_DrawMessageScreen("ENREGISTREMENT AUDIO", "Appuyez sur le bouton", "pour demarrer");
}

void DISPLAY_ShowRecording(void)
{
    DISPLAY_DrawMessageScreen("ENREGISTREMENT AUDIO", "Enregistrement...", 0);
}

void DISPLAY_ShowSaving(void)
{
    DISPLAY_DrawMessageScreen("MEMORISATION AUDIO", "Sauvegarde en Flash...", 0);
}

void DISPLAY_ShowPlayPrompt(void)
{
    DISPLAY_DrawMessageScreen("ERREUR", "Veuillez enregistrer", "un signal d'abord");
}
...
{
    DISPLAY_DrawMessageScreen("LECTURE TERMINEE", "Appuyez pour", "recommencer");
}

void DISPLAY_ShowRecordFinished(void)
{
    DISPLAY_DrawMessageScreen("ENREGISTREMENT", "Enregistrement terminé", 0);
}
...
void DISPLAY_DrawRecordedSignal(const uint16_t *samples, uint32_t sample_count)
{
    ...
    for (index = 0; index < sample_count; index++)
    {
        ...
        ILI9341_DrawFilledRectangle(x, y - 1, x + 1, y + 1, C_ACCENT_TEMP);
    }
}
...
void DISPLAY_DrawMenu(uint8_t selected_index)
{
    ...
    const char *items[] = { "Enregistrer", "Lire audio", "Afficher signal" };
